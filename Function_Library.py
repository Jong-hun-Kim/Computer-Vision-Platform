...
class libCAMERA(object):
    ...
    def color_filtering(self, img, coi=None, print_enable=False):
        self.row, self.col, self.dim = img.shape
        ...
        s_cond = s > SATURATION
        if coi is RED:
            h_cond = (h < HUE_THRESHOLD[coi][0]) | (h > HUE_THRESHOLD[coi][1])
        else:
            h_cond = (h > HUE_THRESHOLD[coi][0]) & (h < HUE_THRESHOLD[coi][1])
    ...
    def object_detection(self, img, sample=0, mode="circle", print_enable=False):
        ...
        for color in (RED, YELLOW, GREEN):
            extract = self.color_filtering(img, coi=color, print_enable=True)
                        ...
                        for idx in range(len(new_lines)):
                            if np.abs(new_lines[:][idx][1] - ya) < VARIANCE:
                                if np.abs(new_lines[:][idx][3] - yb) < VARIANCE:
                                    ...
                                    grad = (xb - xa) / -(yb - ya)  # the fourth quadrant
    ...
    def number_tracking(self, img):
        replica = img.copy()
        self.row, self.col, self.dim = img.shape

        gray_scale = self.gray_conversion(replica)
        self.image_show(gray_scale)
        # Convert Grayscale Image to Binary Image
        ret, thres = cv2.threshold(gray_scale, 127, 255, cv2.THRESH_BINARY)

        contours, hierarchy = cv2.findContours(thres, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)

        for cont in contours:
            np_cont = np.array(cont)
            length = np_cont.shape[0]

            if length < 40:
                continue

            x, y, w, h = cv2.boundingRect(cont)
            cv2.rectangle(replica, (x, y), (x + w, y + h), (255, 0, 0), 3)
            ...
            cv2.circle(replica, (center_x, center_y), 3, (255, 255, 255), -1)
            cv2.drawContours(replica, [cont], -1, (0, 0, 255), 4)
